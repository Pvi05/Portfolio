...
<!-- # Portfolio
An overview of my student &amp; personal projects  -->
...
Congenital Diaphragmatic Hernia is a rare disease affecting about 1 in 3,000 foetuses, characterized by a hole in the diaphragm that allows the stomach, liver and other organs to move into the thorax and compress the heart and lungs. Depending on the severity and timing of the condition, this can cause lung and heart development issues and, in the most serious cases, can be life-threatening. We built and trained a model capable of predicting oxygen requirement at 28 days post-birth with satisfying metrics and strong external validation, in order to further guide doctors' diagnosis.

## A deeper dive into the mission context

Mandated by APHP (Paris' hospital) reference center for the disease, we were asked to explore whether Machine Learning could help doctors better predict whether one foetus would need life-long oxygen support or not. It is an important prediction, as it is made as soon as the disease is diagnosed (usually ~5 months of pregnancy), will guide the entire treatment process and whether IMG (termination of pregnancy) should be advised. Otherwise, a possible treatment is the insertion of a balloon in the lungs of the foetus (FETO) to counteract the compression exerted by the other organs.

Specialists use ultrasound / MRI measurements as criteria (like observed/expected lung-to-head ratio) to assess the foetus condition. They are usually confident about survival rates, but are less able to predict whether the foetus will require long-term respiratory support or only a period of close surveillance, which is our mission to make clearer.

## How the mission went & my contribution

In relation to APHP medical personnel, we proceeded first to make the medical data suitable for ML. In the end, we were left with a proper dataset of ~150 patients. I was responsible for most innovations at this stage, most notably the conversion of medical commentaries as categorical data, the most we could do without requiring NLP.

Due to the small number of samples, simplest methods are best, and we proceeded to train the first models and perform data analysis (feature ranking, various preprocessing, new target columns, ...). At this stage, the medical team's feedback was crucial, as it allowed us to keep our result medically relevant, while our models would also confirm well-installed metrics to rank a foetus condition.

For the final implementation, the key ideas were kept, but made robust and scaled with wider model testing, using the scikit-learn framework to build tailored models (Pipelines, Imputers, Scalers, Encoders, Recursive Feature Selection, Imblearn transformers, ...), and perform methodologically sound model scoring (NestedCrossValidation). Based on F1, Recall (very important in a medical context) and ROC-AUC, the most performant model families were identified to be Random Forest and Logistic Regression, the latter being finally chosen for a better overall trade-off. Scores on unseen data are about 86% F1, 91% recall and 87% ROC-AUC.
I was the one responsible for this last implementation, which was delivered to the hospital.

Alongside the model selection, a graphical interface using Streamlit was developed to make the model easier to use in a medical context. One of the main features of this interface was to include 5 closely related patients from the dataset (using KNN with custom distance) alongside the prediction, allowing the doctor to reflect the current case with past ones.

External data from Louvain's hospital was at last made available to our team, allowing for an external validation of our yet only internally trained model. This external validation yielded strong results for simpler models, close to the original validation metrics, while also highlighting over-fitting of more complex models. Specifically, IterativeImputer showed its limitation, and a standard KNNImputer proved more reliable in generalizing. Results for the LogR with KNNImpute were about 82% ROC-AUC, 89% F1 and 96% recall.
This proved to be an important step in distinguishing models that had truly generalized. It also confirms insights made in previous research works.

## What we delivered

We delivered a complete model training and selection code, as a basis for our final choice, alongside the final model implemented in the interface, ready for clinical use. Although, the medical use is yet to be determined, as the next step would rather be to first compare it in real time with diagnosis on new patients.
I am currently drafting a research article on this work, co-first author with Zakaria El Ouahidi (student) and Pr. Edouard Leyne as supervisor. 
...
Classification, Machine Learning, Ensemble Methods, Decision Tree, Log Regression, Data Treatment & Analysis, Preprocessing methods, Scikit-Learn.
...
<div align='center'>
  <img alt="LinkedIn Example" src="images/LinkedIn1.png" />
  <img alt="LinkedIn Example" src="images/LinkedIn2.png" />
  <img alt="LinkedIn Example" src="images/LinkedIn3.png" />
  <img alt="LinkedIn Example" src="images/LinkedIn4.png" />
  <p><em>A variety of HR manager related jobs titles</em></p>
</div>

## How the mission went & my contribution

Among all the NLP models we tested, we selected DeBERTa-Zero-shot for its precision and its ability to handle rare or unique titles, thanks to semantic capabilities that make it significantly more resilient than a classic supervised classification model. An active learning approach was also considered to compensate for occasional model imprecisions over time, but early results were inconclusive compared to DeBERTa, so it was discontinued.
...
# Building a functional minimal RISC architecture

Using VHDL and Intel's Quartus Suite, I built a functional RISC architecture, describing and linking the main components to form a fully-functional CPU. Alongside this hardware description, I designed an ISA and Assembly to go along and use this CPU.

## The project in detail 

The final architecture implements all the essentials of a Von Neumann-like computer architecture, namely basic arithmetic and bit-wise operations, conditional jumps, RAM interaction, stack interactions, and function handling.
...
- Fetch unit that stores the pc and updates when jumping
...
The project was tested using ModelSim and testbenches, to ensure correct behavior of components and programs. 
...
My implementation did not match the one made by Vadim V. Anshelevich, as it suffered from long calculation times, and could hardly work with the original parameters. Despite these limitations, the quality of its plays showed how relevant searching for game properties can be to replace brute game-tree searching. 
...
I highly recommend the game of Hex for anyone interested in Computer and Game theory, as its properties make it an excellent study case for computer player algorithms, and a great game to try overall.
...
CentraleSupélec experiences a student housing shortage, and finding accommodation on campus can be challenging, especially for second-year students. To help with this problem, I built a bot capable of notifying the user via Discord when free rooms become available. This bot uses Selenium to scrape the student housing website (Césal), with careful cookie management to handle captchas, and, if needed, sends a private Discord message to warn of available rooms. Additionally, multiple options are accessible to the host to customize their experience with the bot (choose which building to track, which discord Ids should receive message, ...)

This bot was used by multiple classmates and students (including myself) to help them find the accommodation they needed.
